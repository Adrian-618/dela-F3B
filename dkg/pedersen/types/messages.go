...
// check that what data structure we need for PVSS.Mostlikely the PubVerShare in pvss is enough
...
// DecPVSSReply is the response of a PVSS decryption request.
//
// - implements serde.Message
...
// Serialize implements serde.Message.
func (resp DecPVSSReply) Serialize(ctx serde.Context) ([]byte, error) {
	format := msgFormats.Get(ctx.GetFormat())
