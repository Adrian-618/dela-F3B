...
func (a runPVSSAction) Execute(ctx node.Context) ([]*pvss.PubVerShare, *share.PubPoly, kyber.Point, kyber.Scalar, error) {
	co, err := getCollectiveAuth(ctx)
	if err != nil {
		return nil, nil, nil, nil, xerrors.Errorf("failed to get collective authority: %v", err)
	}
	...
	err = ctx.Injector.Resolve(&actor)
	if err != nil {
		return nil, nil, nil, nil, xerrors.Errorf("failed to resolve dkg: %v", err)
	}

	t := ctx.Flags.Int("threshold")
	n := co.Len()

	shares, pubpoly, h, symkey, err := actor.RunPVSS(n, t, co)
	//TODO: check where to use the pubpoly and proofs. For now we just return them
	if err != nil {
		return nil, nil, nil, nil, xerrors.Errorf("failed to run pvss: %v", err)
	}
...
type decPVSSAction struct{}

func (a decPVSSAction) Execute(ctx node.Context) error {
	var actor dkg.Actor
